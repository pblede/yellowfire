...
if not exists(select * from rce.club where club_name = 'Randburg Harriers')
	insert into rce.club (district_id, club_name, [version]) values (@district_id, 'Randburg Harriers', 1);
	
declare @club_id bigint;
select @club_id = club_id from rce.club where club_name = 'Randburg Harriers' and district_id = @district_id;

insert into rce.graphic_link (graphic_link_url, graphic_link_image_url, graphic_link_alt_text) values ('http://www.harriers.co.za', 'http://www.harriers.co.za/logo.gif', 'Randburg Harriers');
declare @event_link_id bigint = @@IDENTITY;

insert into rce.graphic_link (graphic_link_url, graphic_link_image_url, graphic_link_alt_text) values ('http://www.asa.org.za', 'http://www.asa.org.za/logo.gif', 'Athletics South Africa');
declare @sponsor_link_id1 bigint = @@IDENTITY;

insert into rce.graphic_link (graphic_link_url, graphic_link_image_url, graphic_link_alt_text) values ('http://www.nike.co.za', 'http://www.nike.co.za/logo.gif', 'Nike');
declare @sponsor_link_id2 bigint = @@IDENTITY;


insert into cde.venue(venue_name, venue_address, venue_gps, [version]) values ('Randburg Harriers Race Club', 'XXXX', '-26.04095564956418, 28.226076364517212', 1);
declare @venue_id bigint = @@IDENTITY;

insert into rce.race (race_name, race_date, venue_id, graphic_link_id, district_id, club_id) values ('Randburg Harriers KFC', '2011-08-01', @venue_id, @event_link_id, @district_id, @club_id);
...
insert into rce.course_category (course_id, race_id, category_name) values (@course_id, @race_id, 'Open Men & Women');
declare @category_id bigint = @@IDENTITY;

insert into rce.category_fee (category_id, course_id, race_id, fee_description, fee_value) values (@category_id, @course_id, @race_id, 'Entry', 79.80);
insert into rce.category_fee (category_id, course_id, race_id, fee_description, fee_value) values (@category_id, @course_id, @race_id, 'Temporary License', 79.80);
...
insert into rce.course_category (course_id, race_id, category_name) values (@course_id, @race_id, '40-49 Men & Women');
 ...
 * Course 2
 */               
insert into rce.course (course_starttime, course_distance, course_description, race_id) values ('07:10', '5km', '5km Run/Walk', @race_id);
set @course_id = @@IDENTITY

insert into rce.course_category (course_id, race_id, category_name) values (@course_id, @race_id, 'Open');
set @category_id = @@IDENTITY;

insert into rce.category_fee (category_id, course_id, race_id, fee_description, fee_value) values (@category_id, @course_id, @race_id, 'Entry', 30.00);
