...
import org.compass.gps.device.jpa.embedded.eclipselink.EclipseLinkEventListener;
 ...
 *
 * <p>Use {@link org.compass.gps.device.jpa.embedded.eclipselink.EclipseLinkHelper} in order to access the <code>Compass</code> instance or the
...
public class CompassSessionCustomizer implements SessionCustomizer {
    ...
    protected PersistenceUnitInfo findPersistenceUnitInfo(Session session) {
        ...
        Map<String, EntityManagerSetupImpl> impls = EntityManagerFactoryProvider.getEmSetupImpls();
        if (impls.containsKey(sessionName)) {
            EntityManagerSetupImpl emSetup = impls.get(sessionName);
            if (log.isDebugEnabled()) {
