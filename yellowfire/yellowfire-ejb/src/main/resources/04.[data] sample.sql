...
select @district_id = district_id from rce.district where district_code = 'GN';

if not exists(select * from rce.club where club_name = 'Harlequin Harriers')
	insert into rce.club (district_id, club_name, [version]) values (@district_id, 'Harlequin Harriers', 1);
	
declare @club_id bigint;
select @club_id = club_id from rce.club where club_name = 'Harlequin Harriers' and district_id = @district_id;


insert into rce.graphic_link (graphic_link_url, graphic_link_image_url, graphic_link_alt_text) values ('http://www.medihelp.co.za', 'http://www.medihelp.co.za/logo.gif', 'Medihelp');
...
insert into cde.venue(venue_name, venue_address, venue_gps, [version]) values ('Harlequin Sports Grounds', 'Harlequin Club, Groenkloof, Pretoria', '-25.767593, 28.217685', 1);
declare @venue_id bigint = @@IDENTITY;

insert into rce.race (race_name, race_date, venue_id, graphic_link_id, district_id, club_id) values ('Medihelp Sunrise Monster 2010', '2011-03-13', @venue_id, @event_link_id, @district_id, @club_id);
 ...
 * Course 1
 */
insert into rce.course (course_starttime, course_distance, course_description, race_id) values ('06:08', '32km', '32km Run', @race_id);
 ...
 * Course 2
 */               
insert into rce.course (course_starttime, course_distance, course_description, race_id) values ('06:30', '10km', '10km Run', @race_id);
 ...
 * Course 3
 */               
insert into rce.course (course_starttime, course_distance, course_description, race_id) values ('06:40', '5km', '5km Fun Run', @race_id);
set @course_id = @@IDENTITY

insert into rce.course_category (course_id, race_id, category_name) values (@course_id, @race_id, 'Open');
set @category_id = @@IDENTITY;

insert into rce.category_fee (category_id, course_id, race_id, fee_description, fee_value) values (@category_id, @course_id, @race_id, 'Entry', 31.00);
